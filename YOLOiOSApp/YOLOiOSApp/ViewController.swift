    ...
    func processDetections(_ result: Any) {
        ...
        for item in items {
            ...
            for child in itemMirror.children {
                if child.label == "label", let val = child.value as? String { label = val.lowercased() }
                if (child.label == "boundingBox" || child.label == "box"), let rect = child.value as? CGRect { maxY = rect.maxY }
            }
        ...
        if shouldAlert {
            ...
            if now - lastAlertTime > 1.2 {
                ...
                AudioServicesPlaySystemSound(1016) // 类似警报音
                lastAlertTime = now
            }
        }
    }
